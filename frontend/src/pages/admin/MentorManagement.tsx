          ...
          <tbody className="divide-y divide-slate-100">
            {mentors.map(m => (
              <tr key={m.id} className="hover:bg-slate-50/50">
                <td className="px-6 py-4 font-medium">{m.firstName} {m.lastName}</td>
                <td className="px-6 py-4">{m.profession}</td>
                <td className="px-6 py-4">{m.company}</td>
                <td className="px-6 py-4">{m.isCertified ? "Yes" : "No"}</td>
                <td className="px-6 py-4">
                  <button onClick={() => deleteMentor(m.id)} className="text-red-500 hover:text-red-700 font-medium">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
